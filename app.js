...
async function submitOpportunity(e){e.preventDefault();if(!requireAuth())return;const button=e.submitter;button.disabled=true;button.textContent='A enviar o ficheiro...';try{const file=$('pMedia').files?.[0],mediaType=await validateOpportunityMedia(file);const mediaUrl=file?await uploadImage(file,'opportunity-images'):null;button.textContent='A publicar...';const body={user_id:state.user.id,type:document.querySelector('input[name="ptype"]:checked').value,title:$('pTitle').value.trim(),category:$('pCategory').value,description:$('pDescription').value.trim(),location:$('pLocation').value.trim(),image_url:mediaType==='image'?mediaUrl:null,video_url:mediaType==='video'?mediaUrl:null,status:'active'};await api('opportunities','',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(body)});e.target.reset();if($('mediaHint'))$('mediaHint').textContent='Fotografia ou vídeo opcional.';toast('Vídeo publicado com sucesso.');await loadPublicData();showView('home')}catch(err){console.error(err);const messages={VIDEO_DURATION:'O vídeo deve ter no máximo 60 segundos.',VIDEO_SIZE:'O vídeo ultrapassa 50 MB. Escolhe um vídeo menor.',IMAGE_SIZE:'A fotografia ultrapassa 10 MB.',UPLOAD_SIZE:'O ficheiro é demasiado grande para o Supabase.',UPLOAD_BUCKET:'O espaço de ficheiros ainda não está configurado.',UPLOAD_PERMISSION:'Sem autorização para enviar. Sai e entra novamente na conta.',UPLOAD_FAILED:'Falha ao enviar o ficheiro. Verifica a internet e tenta novamente.',MEDIA_TYPE:'Formato não suportado. Usa MP4, MOV ou WebM.'};toast(messages[err.message]||'Não foi possível publicar. Executa primeiro o SQL da V13 no Supabase.',true)}finally{button.disabled=false;button.textContent='Publicar agora'}}
