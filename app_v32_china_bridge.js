...
// ===== CHINA DIRETO BRIDGE V32 =====
// Usa o mesmo Supabase REST já utilizado pelo Link Direto.
// A tabela esperada é public.china_status_inbox.
(function(){
  let timer=null;
  let busy=false;
  ...
  async function loadActive(){
    ...
    try{
      ...
      const rows=await api(
        'china_status_inbox',
        `?select=*&expires_at=gt.${encodeURIComponent(now)}&order=created_at.desc&limit=100`
      );
      const data=Array.isArray(rows)?rows:[];
      listeners.forEach(fn=>{try{fn(data)}catch(e){console.warn('China bridge listener',e)}});
      return data;
    }catch(err){
      console.warn('China Direto bridge',err);
      listeners.forEach(fn=>{try{fn([],err)}catch(e){}});
      return [];
    }finally{
      busy=false;
    }
  ...
  function start(intervalMs=8000){
    stop();
    loadActive();
    timer=setInterval(loadActive,Math.max(5000,Number(intervalMs)||8000));
  }
  ...
  window.LinkDiretoChinaBridge={
    loadActive,
    subscribe,
    start,
    stop,
    isAuthenticated:()=>Boolean(state?.session?.access_token && state?.user)
  };

  document.addEventListener('DOMContentLoaded',()=>start(8000));
  window.addEventListener('pageshow',()=>loadActive());
})();
