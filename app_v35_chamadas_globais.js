...
function openMainApp(){
  ...
  loadPublicData();
  setTimeout(()=>{if(typeof ldPollIncoming==='function')ldPollIncoming()},250);
}
...
async function afterLogin(){closeModal();await loadProfile();await loadAdminStatus();await loadFavorites();updateAccountUI();renderProfile();loadPublicData();startNotificationPolling();setTimeout(()=>{if(typeof ldPollIncoming==='function')ldPollIncoming()},150)}
...
async function loadPublicData(){
  ...
  const postProfiles=profilesR.status==='fulfilled'?(profilesR.value||[]):[];
  state.postProfiles=Object.fromEntries(postProfiles.map(p=>[String(p.id),p]));
...
async function registerOpportunityView(o){
  ...
  try{
    const r=await fetch(`${SUPABASE_URL}/rest/v1/rpc/increment_opportunity_view`,{method:'POST',headers:authHeaders(),body:JSON.stringify({p_id:o.id})});
    if(!r.ok)throw new Error(await r.text());
    const value=Number(await r.json().catch(()=>NaN));
    if(Number.isFinite(value))o.views=value;else o.views=opportunityViews(o)+1;
    renderHome();renderSearch();
  }catch(err){console.warn('view count',err);state.viewedOpportunities.delete(String(o.id));}
}
function postSeller(o){
  const p=state.postProfiles?.[String(o.user_id)]||{};
  const name=p.business_name||'Utilizador Link Direto';
  const initials=(name||'LD').split(/\s+/).filter(Boolean).map(x=>x[0]).slice(0,2).join('').toUpperCase()||'LD';
...
function opportunityCard(o){
  ...
  const items=opportunityMediaItems(o),cover=items[0],price=opportunityPrice(o),views=opportunityViews(o);
...
function updatePublishPreview(){const type=document.querySelector('input[name="ptype"]:checked')?.value?.toUpperCase()||'TENHO';const title=$('pTitle')?.value?.trim()||'O teu anúncio aparecerá aqui';const category=$('pCategory')?.value||'Categoria';const location=$('pLocation')?.value?.trim()||'Localização';const price=Number($('pPrice')?.value||0);const priceText=price>0?new Intl.NumberFormat('pt-MZ',{style:'currency',currency:'MZN',maximumFractionDigits:0}).format(price):'';if($('publishPreview'))$('publishPreview').innerHTML=`<small>${esc(type)} • ${esc(category)}</small><b>${esc(title)}</b>${priceText?`<strong>${esc(priceText)}</strong>`:''}<span>📍 ${esc(location)}</span>`}
...
async function submitOpportunity(e){
  ...
  try{
    ...
    const body={user_id:state.user.id,type:document.querySelector('input[name="ptype"]:checked').value,title:$('pTitle').value.trim(),category:$('pCategory').value,description:$('pDescription').value.trim(),price:Number($('pPrice')?.value)||null,views:0,location:$('pLocation').value.trim(),image_url:image_urls[0]||null,image_urls,video_url,status:'active'};
...
function commentsMarkup(id){const items=commentsFor(id);return items.map(c=>{const mine=state.user?.id===c.user_id;return `<div class="comment"><div><b>${esc(c.author_name||'Utilizador')}</b><small>${formatDate(c.created_at)}</small></div><p>${esc(c.body)}</p>${mine?`<div class="comment-owner-actions"><button class="btn ghost" type="button" onclick="openCommentActions('${c.id}','${id}')">⋯ Opções</button></div>`:''}</div>`}).join('')||'<div class="empty">Ainda não existem comentários. Sê o primeiro.</div>'}
...
function openCommentActions(commentId,opportunityId){const c=state.comments.find(x=>String(x.id)===String(commentId));if(!c||!state.user||c.user_id!==state.user.id)return;openModal(`<h2>Opções do comentário</h2><div class="message-action-preview">${esc(c.body)}</div><div class="message-actions"><button class="btn secondary" type="button" onclick="openEditComment('${commentId}','${opportunityId}')">✏️ Editar comentário</button><button class="btn danger" type="button" onclick="deleteComment('${commentId}','${opportunityId}')">🗑️ Eliminar comentário</button><button class="btn ghost" type="button" onclick="openComments('${opportunityId}')">Cancelar</button></div>`)}
function openEditComment(commentId,opportunityId){const c=state.comments.find(x=>String(x.id)===String(commentId));if(!c||!state.user||c.user_id!==state.user.id)return;openModal(`<h2>Editar comentário</h2><form onsubmit="saveEditedComment(event,'${commentId}','${opportunityId}')"><label>Comentário<textarea id="editCommentBody" required maxlength="500">${esc(c.body||'')}</textarea></label><button class="btn primary">Guardar alteração</button><button class="btn ghost" type="button" onclick="openComments('${opportunityId}')">Cancelar</button></form>`);setTimeout(()=>$('editCommentBody')?.focus(),50)}
async function saveEditedComment(e,commentId,opportunityId){e.preventDefault();if(!requireAuth())return;const body=$('editCommentBody')?.value.trim();if(!body)return;const button=e.submitter;button.disabled=true;button.textContent='A guardar...';try{await api('opportunity_comments',`?id=eq.${commentId}&user_id=eq.${state.user.id}`,{method:'PATCH',body:JSON.stringify({body})});const c=state.comments.find(x=>String(x.id)===String(commentId));if(c)c.body=body;openComments(opportunityId);renderHome();renderSearch();toast('Comentário editado.')}catch(err){console.error(err);toast('Não foi possível editar o comentário.',true);button.disabled=false;button.textContent='Guardar alteração'}}
async function deleteComment(commentId,opportunityId){if(!confirm('Eliminar este comentário?'))return;try{await api('opportunity_comments',`?id=eq.${commentId}&user_id=eq.${state.user.id}`,{method:'DELETE'});state.comments=state.comments.filter(x=>String(x.id)!==String(commentId));openComments(opportunityId);renderHome();renderSearch();toast('Comentário eliminado.')}catch{toast('Não foi possível eliminar o comentário.',true)}}
function manageOpportunity(id){const o=state.opportunities.find(x=>String(x.id)===String(id));if(!o||!state.user||o.user_id!==state.user.id){toast('Só podes gerir as tuas próprias publicações.',true);return}openModal(`<h2>Gerir publicação</h2><h3>${esc(o.title)}</h3><p>${esc(o.description)}</p><button class="btn primary" type="button" onclick="openEditOpportunity('${id}')">✏️ Editar publicação</button><button class="btn secondary" type="button" onclick="setOpportunityStatus('${id}','${o.status==='closed'?'active':'closed'}')">${o.status==='closed'?'Reabrir':'Marcar como concluída'}</button><button class="btn danger" type="button" onclick="deleteOpportunity('${id}')">🗑️ Eliminar publicação</button><button class="btn ghost" type="button" onclick="closeModal()">Cancelar</button>`)}
function openEditOpportunity(id){const o=state.opportunities.find(x=>String(x.id)===String(id));if(!o||!state.user||o.user_id!==state.user.id)return;const options=CATEGORIES.map(c=>`<option ${c===o.category?'selected':''}>${esc(c)}</option>`).join('');openModal(`<h2>Editar publicação</h2><form onsubmit="saveEditedOpportunity(event,'${id}')"><label>Tipo<select id="editOpportunityType"><option value="tenho" ${o.type==='tenho'?'selected':''}>TENHO</option><option value="preciso" ${o.type==='preciso'?'selected':''}>PRECISO</option></select></label><label>Título<input id="editOpportunityTitle" required maxlength="120" value="${esc(o.title||'')}"></label><label>Categoria<select id="editOpportunityCategory">${options}</select></label><label>Descrição<textarea id="editOpportunityDescription" required maxlength="2000">${esc(o.description||'')}</textarea></label><label>Preço (opcional)<input id="editOpportunityPrice" type="number" inputmode="decimal" min="0" step="1" value="${Number(o.price)||''}"></label><label>Localização<input id="editOpportunityLocation" required maxlength="120" value="${esc(o.location||'')}"></label><p class="muted">As fotos e o vídeo atuais serão mantidos.</p><button class="btn primary">Guardar alterações</button><button class="btn ghost" type="button" onclick="manageOpportunity('${id}')">Cancelar</button></form>`);setTimeout(()=>$('editOpportunityTitle')?.focus(),50)}
async function saveEditedOpportunity(e,id){e.preventDefault();if(!requireAuth())return;const o=state.opportunities.find(x=>String(x.id)===String(id));if(!o||o.user_id!==state.user.id)return;const button=e.submitter;button.disabled=true;button.textContent='A guardar...';const body={type:$('editOpportunityType').value,title:$('editOpportunityTitle').value.trim(),category:$('editOpportunityCategory').value,description:$('editOpportunityDescription').value.trim(),price:Number($('editOpportunityPrice').value)||null,location:$('editOpportunityLocation').value.trim()};try{await api('opportunities',`?id=eq.${id}&user_id=eq.${state.user.id}`,{method:'PATCH',body:JSON.stringify(body)});Object.assign(o,body);closeModal();renderHome();renderSearch();toast('Publicação editada com sucesso.')}catch(err){console.error(err);toast('Não foi possível editar a publicação.',true);button.disabled=false;button.textContent='Guardar alterações'}}
async function setOpportunityStatus(id,status){try{await api('opportunities',`?id=eq.${id}&user_id=eq.${state.user.id}`,{method:'PATCH',body:JSON.stringify({status})});closeModal();await loadPublicData();toast('Publicação atualizada.')}catch{toast('Não foi possível atualizar.',true)}}
async function deleteOpportunity(id){if(!confirm('Eliminar esta publicação?'))return;try{await api('opportunities',`?id=eq.${id}&user_id=eq.${state.user.id}`,{method:'DELETE'});closeModal();await loadPublicData();toast('Publicação eliminada.')}catch{toast('Não foi possível eliminar.',true)}}
...
async function openChat(proposalId,otherId){const proposal=state.proposals.find(p=>String(p.id)===String(proposalId));try{const [messages,calls]=await Promise.all([api('messages',`?proposal_id=eq.${proposalId}&order=created_at.asc`),api('call_sessions',`?proposal_id=eq.${proposalId}&order=created_at.desc&limit=12`).catch(()=>[])]);state.messages=messages||[];state.callHistory=calls||[];openModal(`<div class="ld-chat-head"><div><h2>Sala de Negócio</h2><p>${esc(proposal?.opportunity_title||'Conversa')}</p></div><div class="ld-chat-call-actions"><button type="button" class="ld-chat-call-btn" onclick="ldStartCall('voice','${proposalId}','${otherId}')" aria-label="Chamada de voz">📞</button><button type="button" class="ld-chat-call-btn" onclick="ldStartCall('video','${proposalId}','${otherId}')" aria-label="Videochamada">🎥</button></div></div><div class="chat-note">As mensagens ficam guardadas até serem apagadas.</div>${ldCallHistoryMarkup(proposalId)}<div id="chatMessages" class="chat">${renderMessages(proposalId,otherId)}</div><form onsubmit="sendMessage(event,'${proposalId}','${otherId}')"><label>Mensagem<input id="chatInput" required autocomplete="off" maxlength="2000" placeholder="Escreve uma mensagem..."></label><button class="btn primary">Enviar</button></form>`);setTimeout(()=>{const c=$('chatMessages');if(c)c.scrollTop=c.scrollHeight},0)}catch(err){console.error(err);toast('Não foi possível abrir a conversa. Executa o SQL da V14 no Supabase.',true)}}
...
// ===== LINK DIRETO CALLS V33 — toque + não atendidas + contador só depois de atender =====
const LD_CALL_CONFIG={iceServers:[{urls:'stun:stun.l.google.com:19302'},{urls:'stun:stun1.l.google.com:19302'}],iceCandidatePoolSize:4};
const LD_CALL_RING_TIMEOUT_MS=35000;
const ldCall={current:null,incoming:null,pollTimer:null,sessionTimer:null,iceTimer:null,ringTimeout:null,seenIncoming:new Set(),started:false,audioCtx:null,toneTimer:null,vibrateTimer:null};

(function initLdCallStyles(){
  if(document.getElementById('ld-call-v33-styles'))return;
  const st=document.createElement('style');st.id='ld-call-v33-styles';st.textContent=`
  .ld-chat-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;padding-right:4px}.ld-chat-head h2{margin:0}.ld-chat-head p{margin:4px 0 0;color:var(--muted,#93a099)}
  .ld-chat-call-actions{display:flex;gap:7px;flex:none;margin-right:34px}.ld-chat-call-btn{width:40px;height:40px;min-width:40px;border-radius:50%;border:1px solid rgba(45,245,176,.28);background:rgba(45,245,176,.11);color:#fff;font-size:18px;display:flex;align-items:center;justify-content:center;padding:0}.ld-chat-call-btn:active{transform:scale(.94)}
  .ld-call-history{margin:8px 0 12px;border:1px solid rgba(255,255,255,.08);border-radius:13px;overflow:hidden;background:rgba(255,255,255,.025)}.ld-call-history-title{font-size:10px;font-weight:800;letter-spacing:.08em;color:#93a099;padding:9px 11px 6px}.ld-call-history-row{display:flex;align-items:center;gap:9px;padding:9px 11px;border-top:1px solid rgba(255,255,255,.055);font-size:11px}.ld-call-history-row:first-of-type{border-top:0}.ld-call-history-row .ico{font-size:15px}.ld-call-history-row .copy{min-width:0;flex:1}.ld-call-history-row b{display:block;font-size:11px}.ld-call-history-row small{display:block;color:#8f9a95;margin-top:2px}.ld-call-history-row.missed b{color:#ff7777}.ld-call-history-row.rejected b{color:#ffb06b}
  #ldCallOverlay{position:fixed;inset:0;z-index:2147483000;background:radial-gradient(circle at 50% 18%,#183129 0,#08100d 42%,#030504 100%);color:#fff;display:none;flex-direction:column;align-items:center;justify-content:space-between;padding:calc(38px + env(safe-area-inset-top)) 18px calc(30px + env(safe-area-inset-bottom));box-sizing:border-box}
  #ldCallOverlay.show{display:flex}.ld-call-person{text-align:center;margin-top:3vh}.ld-call-avatar{width:86px;height:86px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 15px;background:#18382d;border:2px solid rgba(100,255,196,.25);font-size:30px;font-weight:900}.ld-call-person h2{margin:0 0 7px;font-size:24px}.ld-call-person p{margin:0;color:#a9b8b1}.ld-call-video-stage{position:absolute;inset:0;z-index:-1;overflow:hidden}.ld-call-video-stage video.remote{width:100%;height:100%;object-fit:cover;background:#050806}.ld-call-video-stage video.local{position:absolute;right:14px;top:calc(15px + env(safe-area-inset-top));width:104px;height:142px;object-fit:cover;border-radius:16px;border:1px solid rgba(255,255,255,.22);background:#111;box-shadow:0 8px 30px rgba(0,0,0,.35)}.ld-call-video-stage.video-on{z-index:0}.ld-call-video-stage.video-on~.ld-call-person{position:relative;z-index:2;text-shadow:0 1px 8px #000}.ld-call-controls{display:flex;align-items:center;justify-content:center;gap:18px;position:relative;z-index:4}.ld-call-round{width:58px;height:58px;border-radius:50%;border:0;background:rgba(255,255,255,.15);color:#fff;font-size:22px}.ld-call-round.off{opacity:.5}.ld-call-round.hang{background:#e34747;width:66px;height:66px}.ld-call-incoming-actions{display:flex;gap:38px;position:relative;z-index:4}.ld-call-answer{background:#21c77a}.ld-call-reject{background:#e34747}.ld-call-status-pill{margin-top:14px;font-size:13px;color:#c6d2cd;min-height:18px}.ld-call-hidden{display:none!important}
  `;document.head.appendChild(st);
})();

function ldCallHistoryMarkup(proposalId){
  const rows=(state.callHistory||[]).filter(x=>String(x.proposal_id)===String(proposalId)).slice(0,5);if(!rows.length)return'';
  const body=rows.map(r=>{const mine=r.caller_id===state.user?.id;const dir=mine?'Efetuada':'Recebida';let label='Chamada';let cls='';if(r.status==='missed'){label=mine?'Sem resposta':'Chamada não atendida';cls='missed'}else if(r.status==='rejected'){label=mine?'Chamada rejeitada':'Chamada recusada';cls='rejected'}else if(r.status==='ended'&&r.answered_at){const s=Math.max(0,Math.round((new Date(r.ended_at||Date.now())-new Date(r.answered_at))/1000));label=`${dir} · ${Math.floor(s/60)}:${String(s%60).padStart(2,'0')}`}else if(r.status==='accepted')label=`${dir} · em curso`;else label=dir;const when=new Date(r.created_at).toLocaleString('pt-PT',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});return`<div class="ld-call-history-row ${cls}"><span class="ico">${r.call_type==='video'?'🎥':'📞'}</span><span class="copy"><b>${label}</b><small>${when}</small></span></div>`}).join('');
  return`<div class="ld-call-history"><div class="ld-call-history-title">CHAMADAS</div>${body}</div>`;
}
function ldCallEnsureOverlay(){let el=document.getElementById('ldCallOverlay');if(el)return el;el=document.createElement('div');el.id='ldCallOverlay';el.innerHTML=`<div class="ld-call-video-stage" id="ldCallVideoStage"><video id="ldRemoteVideo" class="remote" autoplay playsinline></video><video id="ldLocalVideo" class="local" autoplay playsinline muted></video></div><div class="ld-call-person"><div class="ld-call-avatar" id="ldCallAvatar">LD</div><h2 id="ldCallTitle">Chamada</h2><p id="ldCallStatus">A preparar...</p><small id="ldCallGlobalHint" style="display:none;margin-top:8px;color:#80f0c0;font-weight:700">CHAMADA RECEBIDA</small><div class="ld-call-status-pill" id="ldCallTimer"></div></div><div class="ld-call-incoming-actions ld-call-hidden" id="ldIncomingActions"><button class="ld-call-round ld-call-reject" onclick="ldRejectIncoming()">✕</button><button class="ld-call-round ld-call-answer" onclick="ldAcceptIncoming()">📞</button></div><div class="ld-call-controls ld-call-hidden" id="ldActiveControls"><button class="ld-call-round" id="ldMuteBtn" onclick="ldToggleMute()">🎙️</button><button class="ld-call-round hang" onclick="ldEndCall(true)">✕</button><button class="ld-call-round" id="ldCameraBtn" onclick="ldToggleCamera()">📹</button></div>`;document.body.appendChild(el);return el}
function ldCallName(userId){const p=state.postProfiles?.[String(userId)]||state.discoverProfiles?.find?.(x=>String(x.id)===String(userId));return p?.business_name||'Link Direto'}
function ldCallInitials(name='LD'){return String(name).split(/\s+/).filter(Boolean).map(x=>x[0]).slice(0,2).join('').toUpperCase()||'LD'}
function ldCallShow({name='Link Direto',status='A chamar...',incoming=false,video=false}={}){const el=ldCallEnsureOverlay();$('ldCallTitle').textContent=name;$('ldCallAvatar').textContent=ldCallInitials(name);$('ldCallStatus').textContent=status;$('ldCallTimer').textContent='';const gh=$('ldCallGlobalHint');if(gh)gh.style.display=incoming?'block':'none';$('ldIncomingActions').classList.toggle('ld-call-hidden',!incoming);$('ldActiveControls').classList.toggle('ld-call-hidden',incoming);$('ldCameraBtn').classList.toggle('ld-call-hidden',!video);$('ldCallVideoStage').classList.toggle('video-on',video);el.classList.add('show')}
function ldCallHide(){document.getElementById('ldCallOverlay')?.classList.remove('show')}
function ldCallSetStatus(t){const e=$('ldCallStatus');if(e)e.textContent=t}
function ldCallStartTimer(answeredAt){if(!ldCall.current||ldCall.current.connectedAt)return;const base=answeredAt?new Date(answeredAt).getTime():Date.now();ldCall.current.connectedAt=Number.isFinite(base)?base:Date.now();clearInterval(ldCall.current.timer);const tick=()=>{if(!ldCall.current?.connectedAt)return;const sec=Math.max(0,Math.floor((Date.now()-ldCall.current.connectedAt)/1000)),m=Math.floor(sec/60),s=String(sec%60).padStart(2,'0');if($('ldCallTimer'))$('ldCallTimer').textContent=`${m}:${s}`};tick();ldCall.current.timer=setInterval(tick,1000)}
function ldCallStopTimers(){clearInterval(ldCall.sessionTimer);clearInterval(ldCall.iceTimer);clearTimeout(ldCall.ringTimeout);ldCall.sessionTimer=null;ldCall.iceTimer=null;ldCall.ringTimeout=null;if(ldCall.current?.timer)clearInterval(ldCall.current.timer)}
function ldToneContext(){try{if(!ldCall.audioCtx)ldCall.audioCtx=new (window.AudioContext||window.webkitAudioContext)();if(ldCall.audioCtx.state==='suspended')ldCall.audioCtx.resume();return ldCall.audioCtx}catch{return null}}
function ldTone(freq=440,duration=.18,volume=.035){const ctx=ldToneContext();if(!ctx)return;const o=ctx.createOscillator(),g=ctx.createGain();o.frequency.value=freq;o.type='sine';g.gain.setValueAtTime(0,ctx.currentTime);g.gain.linearRampToValueAtTime(volume,ctx.currentTime+.015);g.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+duration);o.connect(g).connect(ctx.destination);o.start();o.stop(ctx.currentTime+duration+.02)}
function ldStartRing(kind='incoming'){ldStopRing();const play=()=>{if(kind==='incoming'){ldTone(620,.22,.055);setTimeout(()=>ldTone(760,.22,.05),270);if(navigator.vibrate)navigator.vibrate([350,180,350])}else{ldTone(440,.18,.025);setTimeout(()=>ldTone(480,.18,.022),250)}};play();ldCall.toneTimer=setInterval(play,kind==='incoming'?1800:2300)}
function ldStopRing(){clearInterval(ldCall.toneTimer);ldCall.toneTimer=null;if(navigator.vibrate)navigator.vibrate(0)}
async function ldCallMedia(kind){return navigator.mediaDevices.getUserMedia({audio:true,video:kind==='video'?{facingMode:'user'}:false})}
function ldCallAttachStream(stream){const v=$('ldLocalVideo');if(v)v.srcObject=stream}
function ldCallAttachRemote(stream){const v=$('ldRemoteVideo');if(v)v.srcObject=stream}
async function ldCallCreatePeer(callId,localStream){const pc=new RTCPeerConnection(LD_CALL_CONFIG),remote=new MediaStream();ldCallAttachRemote(remote);localStream.getTracks().forEach(t=>pc.addTrack(t,localStream));pc.ontrack=e=>e.streams[0]?.getTracks().forEach(t=>{if(!remote.getTracks().some(x=>x.id===t.id))remote.addTrack(t)});pc.onicecandidate=e=>{if(!e.candidate||!state.user||!callId)return;api('call_ice_candidates','',{method:'POST',body:JSON.stringify({call_id:callId,sender_id:state.user.id,candidate:e.candidate.toJSON()})}).catch(err=>console.warn('call ice',err))};pc.onconnectionstatechange=()=>{if(!ldCall.current||ldCall.current.id!==callId)return;if(pc.connectionState==='connected'){ldStopRing();ldCallSetStatus('Ligado')}if(['failed','closed'].includes(pc.connectionState))ldEndCall(false)};return pc}
async function ldStartCall(kind,proposalId,otherId){if(!requireAuth())return;if(ldCall.current||ldCall.incoming){toast('Já existe uma chamada em curso.',true);return}if(!navigator.mediaDevices?.getUserMedia||!window.RTCPeerConnection){toast('Este navegador não suporta chamadas.',true);return}ldToneContext();const name=ldCallName(otherId);ldCallShow({name,status:'A pedir acesso ao microfone...',video:kind==='video'});try{const stream=await ldCallMedia(kind);ldCallAttachStream(stream);const created=await api('call_sessions','',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({proposal_id:proposalId,caller_id:state.user.id,callee_id:otherId,call_type:kind,status:'preparing'})});const row=created?.[0];if(!row?.id)throw new Error('CALL_CREATE');ldCall.current={id:row.id,proposalId,otherId,kind,role:'caller',stream,pc:null,remoteIce:new Set(),connectedAt:null,answered:false};const pc=await ldCallCreatePeer(row.id,stream);ldCall.current.pc=pc;const offer=await pc.createOffer();await pc.setLocalDescription(offer);await api('call_sessions',`?id=eq.${row.id}`,{method:'PATCH',body:JSON.stringify({offer:pc.localDescription,status:'ringing'})});ldCallSetStatus('A chamar...');ldStartRing('outgoing');ldCallStartSessionPoll();ldCallStartIcePoll();ldCall.ringTimeout=setTimeout(()=>ldMarkMissed(),LD_CALL_RING_TIMEOUT_MS)}catch(err){console.error('start call',err);ldCallCleanupLocal();toast(err?.name==='NotAllowedError'?'Autoriza o microfone/câmara para fazer a chamada.':`Falha na chamada: ${String(err?.message||err||'erro desconhecido').slice(0,180)}`,true)}}
async function ldMarkMissed(){const c=ldCall.current;if(!c||c.answered)return;try{await api('call_sessions',`?id=eq.${c.id}&status=eq.ringing`,{method:'PATCH',body:JSON.stringify({status:'missed',ended_at:new Date().toISOString()})})}catch{}toast('Chamada não atendida.');ldCallCleanupLocal()}
async function ldPollIncoming(){if(!state.user||ldCall.current||ldCall.incoming)return;try{const rows=await api('call_sessions',`?callee_id=eq.${state.user.id}&status=eq.ringing&order=created_at.desc&limit=1`),row=rows?.[0];if(!row||ldCall.seenIncoming.has(String(row.id)))return;const age=Date.now()-new Date(row.created_at).getTime();if(age>LD_CALL_RING_TIMEOUT_MS){try{await api('call_sessions',`?id=eq.${row.id}&status=eq.ringing`,{method:'PATCH',body:JSON.stringify({status:'missed',ended_at:new Date().toISOString()})})}catch{}return}ldCall.seenIncoming.add(String(row.id));ldCall.incoming=row;const name=ldCallName(row.caller_id);ldCallShow({name,status:row.call_type==='video'?'Videochamada recebida':'Chamada de voz recebida',incoming:true,video:false});ldStartRing('incoming');ldCall.ringTimeout=setTimeout(async()=>{if(ldCall.incoming?.id!==row.id)return;try{await api('call_sessions',`?id=eq.${row.id}&status=eq.ringing`,{method:'PATCH',body:JSON.stringify({status:'missed',ended_at:new Date().toISOString()})})}catch{}ldCall.incoming=null;ldStopRing();ldCallHide();toast('Chamada não atendida.')},Math.max(1000,LD_CALL_RING_TIMEOUT_MS-age));if('Notification'in window&&Notification.permission==='granted'&&document.hidden){try{new Notification('Chamada no Link Direto',{body:`${name} está a ligar.`})}catch{}}}catch(err){if(!String(err).includes('404'))console.warn('incoming call poll',err)}}
async function ldAcceptIncoming(){const row=ldCall.incoming;if(!row||!state.user)return;clearTimeout(ldCall.ringTimeout);ldStopRing();const name=ldCallName(row.caller_id);ldCallShow({name,status:'A estabelecer ligação...',incoming:false,video:row.call_type==='video'});try{const stream=await ldCallMedia(row.call_type);ldCallAttachStream(stream);ldCall.current={id:row.id,proposalId:row.proposal_id,otherId:row.caller_id,kind:row.call_type,role:'callee',stream,pc:null,remoteIce:new Set(),connectedAt:null,answered:true};ldCall.incoming=null;const pc=await ldCallCreatePeer(row.id,stream);ldCall.current.pc=pc;await pc.setRemoteDescription(new RTCSessionDescription(row.offer));const answer=await pc.createAnswer();await pc.setLocalDescription(answer);const answeredAt=new Date().toISOString();await api('call_sessions',`?id=eq.${row.id}`,{method:'PATCH',body:JSON.stringify({answer:pc.localDescription,status:'accepted',answered_at:answeredAt})});ldCallStartTimer(answeredAt);ldCallStartSessionPoll();ldCallStartIcePoll()}catch(err){console.error('accept call',err);toast(err?.name==='NotAllowedError'?'Autoriza o microfone/câmara para atender.':'Não foi possível atender.',true);await ldEndCall(true)}}
async function ldRejectIncoming(){const row=ldCall.incoming;if(!row)return;clearTimeout(ldCall.ringTimeout);ldStopRing();ldCall.incoming=null;ldCallHide();try{await api('call_sessions',`?id=eq.${row.id}`,{method:'PATCH',body:JSON.stringify({status:'rejected',ended_at:new Date().toISOString()})})}catch{}}
function ldCallStartSessionPoll(){clearInterval(ldCall.sessionTimer);ldCall.sessionTimer=setInterval(ldPollCurrentSession,1100);ldPollCurrentSession()}
async function ldPollCurrentSession(){const c=ldCall.current;if(!c||!state.user)return;try{const rows=await api('call_sessions',`?id=eq.${c.id}&limit=1`),row=rows?.[0];if(!row)return;if(c.role==='caller'&&row.status==='accepted'&&row.answer&&!c.pc.remoteDescription){clearTimeout(ldCall.ringTimeout);ldStopRing();c.answered=true;await c.pc.setRemoteDescription(new RTCSessionDescription(row.answer));ldCallStartTimer(row.answered_at);ldCallSetStatus('A estabelecer ligação...')}if(row.status==='rejected'){ldStopRing();toast('Chamada rejeitada.');ldCallCleanupLocal()}if(row.status==='missed'){ldStopRing();toast('Chamada não atendida.');ldCallCleanupLocal()}if(row.status==='ended')ldCallCleanupLocal()}catch(err){console.warn('call session poll',err)}}
function ldCallStartIcePoll(){clearInterval(ldCall.iceTimer);ldCall.iceTimer=setInterval(ldPollRemoteIce,800);ldPollRemoteIce()}
async function ldPollRemoteIce(){const c=ldCall.current;if(!c?.pc||!state.user)return;try{const rows=await api('call_ice_candidates',`?call_id=eq.${c.id}&sender_id=neq.${state.user.id}&order=created_at.asc&limit=100`)||[];for(const r of rows){const key=String(r.id);if(c.remoteIce.has(key))continue;c.remoteIce.add(key);try{await c.pc.addIceCandidate(new RTCIceCandidate(r.candidate))}catch(err){console.warn('add ice',err)}}}catch(err){console.warn('ice poll',err)}}
async function ldEndCall(notify=true){const c=ldCall.current;if(!c){if(ldCall.incoming)return ldRejectIncoming();ldCallHide();return}ldStopRing();if(notify){try{const status=c.answered?'ended':'missed';await api('call_sessions',`?id=eq.${c.id}`,{method:'PATCH',body:JSON.stringify({status,ended_at:new Date().toISOString()})})}catch{}}ldCallCleanupLocal()}
function ldCallCleanupLocal(){ldCallStopTimers();ldStopRing();const c=ldCall.current;if(c?.pc)try{c.pc.close()}catch{};if(c?.stream)c.stream.getTracks().forEach(t=>t.stop());ldCall.current=null;ldCall.incoming=null;const rv=$('ldRemoteVideo'),lv=$('ldLocalVideo');if(rv)rv.srcObject=null;if(lv)lv.srcObject=null;ldCallHide()}
function ldToggleMute(){const c=ldCall.current;if(!c?.stream)return;const tracks=c.stream.getAudioTracks();tracks.forEach(t=>t.enabled=!t.enabled);const muted=tracks.length&&!tracks[0].enabled;$('ldMuteBtn')?.classList.toggle('off',muted)}
function ldToggleCamera(){const c=ldCall.current;if(!c?.stream)return;const tracks=c.stream.getVideoTracks();tracks.forEach(t=>t.enabled=!t.enabled);const off=tracks.length&&!tracks[0].enabled;$('ldCameraBtn')?.classList.toggle('off',off)}
function ldInitCalls(){
  if(ldCall.started)return;
  ldCall.started=true;
  ldCallEnsureOverlay();
  // Escuta chamadas em qualquer ecrã da app, não apenas no Connect/chat.
  ldCall.pollTimer=setInterval(ldPollIncoming,1200);
  setTimeout(ldPollIncoming,100);
  window.addEventListener('focus',ldPollIncoming);
  window.addEventListener('pageshow',ldPollIncoming);
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)ldPollIncoming()});
  window.addEventListener('beforeunload',()=>{if(ldCall.current)ldEndCall(true)});
  document.addEventListener('pointerdown',()=>ldToneContext(),{once:true});
}
// Funciona tanto se o JS carregar antes como depois do DOMContentLoaded.
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',ldInitCalls);
else ldInitCalls();
