...
(function initLdCallStyles(){
  ...
  #ldCallOverlay{position:fixed;inset:0;z-index:30000;background:radial-gradient(circle at 50% 18%,#183129 0,#08100d 42%,#030504 100%);color:#fff;display:none;flex-direction:column;align-items:center;justify-content:space-between;padding:calc(38px + env(safe-area-inset-top)) 18px calc(30px + env(safe-area-inset-bottom));box-sizing:border-box}
...
function ldCallShow({name='Link Direto',status='A chamar...',incoming=false,video=false}={}){const el=ldCallEnsureOverlay();$('ldCallTitle').textContent=name;$('ldCallAvatar').textContent=ldCallInitials(name);$('ldCallStatus').textContent=status;$('ldCallTimer').textContent='';$('ldIncomingActions').classList.toggle('ld-call-hidden',!incoming);$('ldActiveControls').classList.toggle('ld-call-hidden',incoming);$('ldCameraBtn').classList.toggle('ld-call-hidden',!video);$('ldCallVideoStage').classList.toggle('video-on',video);el.classList.add('show')}
...
function ldInitCalls(){if(ldCall.started)return;ldCall.started=true;ldCallEnsureOverlay();ldCall.pollTimer=setInterval(ldPollIncoming,1800);window.addEventListener('beforeunload',()=>{if(ldCall.current)ldEndCall(true)});document.addEventListener('visibilitychange',()=>{if(!document.hidden)ldPollIncoming()});document.addEventListener('pointerdown',()=>ldToneContext(),{once:true})}
document.addEventListener('DOMContentLoaded',ldInitCalls);
