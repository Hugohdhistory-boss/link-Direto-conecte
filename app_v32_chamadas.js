...
// ===== LINK DIRETO CALLS V32 — WebRTC + sinalização Supabase REST =====
const LD_CALL_CONFIG={
  iceServers:[
...
const ldCall={current:null,incoming:null,pollTimer:null,sessionTimer:null,iceTimer:null,seenIncoming:new Set(),started:false};

(function initLdCallStyles(){
  if(document.getElementById('ld-call-v32-styles'))return;
  const st=document.createElement('style');st.id='ld-call-v32-styles';st.textContent=`
...
function ldCallStartTimer(){if(!ldCall.current)return;ldCall.current.connectedAt=Date.now();clearInterval(ldCall.current.timer);ldCall.current.timer=setInterval(()=>{if(!ldCall.current?.connectedAt)return;const sec=Math.floor((Date.now()-ldCall.current.connectedAt)/1000),m=Math.floor(sec/60),s=String(sec%60).padStart(2,'0');if($('ldCallTimer'))$('ldCallTimer').textContent=`${m}:${s}`},1000)}
function ldCallStopTimers(){clearInterval(ldCall.sessionTimer);clearInterval(ldCall.iceTimer);ldCall.sessionTimer=null;ldCall.iceTimer=null;if(ldCall.current?.timer)clearInterval(ldCall.current.timer)}
...
async function ldCallCreatePeer(callId,localStream){
  const pc=new RTCPeerConnection(LD_CALL_CONFIG);const remote=new MediaStream();ldCallAttachRemote(remote);localStream.getTracks().forEach(t=>pc.addTrack(t,localStream));
  pc.ontrack=e=>e.streams[0]?.getTracks().forEach(t=>{if(!remote.getTracks().some(x=>x.id===t.id))remote.addTrack(t)});
  pc.onicecandidate=e=>{if(!e.candidate||!state.user||!callId)return;api('call_ice_candidates','',{method:'POST',body:JSON.stringify({call_id:callId,sender_id:state.user.id,candidate:e.candidate.toJSON()})}).catch(err=>console.warn('call ice',err))};
  pc.onconnectionstatechange=()=>{if(!ldCall.current||ldCall.current.id!==callId)return;if(pc.connectionState==='connected'){ldCallSetStatus('Ligado');if(!ldCall.current.connectedAt)ldCallStartTimer()}if(['failed','closed'].includes(pc.connectionState))ldEndCall(false)};
  return pc;
}
async function ldStartCall(kind,proposalId,otherId){
  if(!requireAuth())return;if(ldCall.current||ldCall.incoming){toast('Já existe uma chamada em curso.',true);return}if(!navigator.mediaDevices?.getUserMedia||!window.RTCPeerConnection){toast('Este navegador não suporta chamadas.',true);return}
  const name=ldCallName(otherId);ldCallShow({name,status:'A pedir acesso ao microfone...',video:kind==='video'});
  try{
    const stream=await ldCallMedia(kind);ldCallAttachStream(stream);
    const created=await api('call_sessions','',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({proposal_id:proposalId,caller_id:state.user.id,callee_id:otherId,call_type:kind,status:'preparing'})});
    const row=created?.[0];if(!row?.id)throw new Error('CALL_CREATE');
    ldCall.current={id:row.id,proposalId,otherId,kind,role:'caller',stream,pc:null,remoteIce:new Set(),connectedAt:null};
    const pc=await ldCallCreatePeer(row.id,stream);ldCall.current.pc=pc;const offer=await pc.createOffer();await pc.setLocalDescription(offer);
    await api('call_sessions',`?id=eq.${row.id}`,{method:'PATCH',body:JSON.stringify({offer:pc.localDescription,status:'ringing'})});
    ldCallSetStatus('A chamar...');ldCallStartSessionPoll();ldCallStartIcePoll();
  }catch(err){console.error('start call',err);ldCallCleanupLocal();toast(err?.name==='NotAllowedError'?'Autoriza o microfone/câmara para fazer a chamada.':'Não foi possível iniciar a chamada. Confirma o SQL de chamadas no Supabase.',true)}
}
async function ldPollIncoming(){
  if(!state.user||ldCall.current||ldCall.incoming)return;
  try{const rows=await api('call_sessions',`?callee_id=eq.${state.user.id}&status=eq.ringing&order=created_at.desc&limit=1`);const row=rows?.[0];if(!row||ldCall.seenIncoming.has(String(row.id)))return;ldCall.seenIncoming.add(String(row.id));ldCall.incoming=row;const name=ldCallName(row.caller_id);ldCallShow({name,status:row.call_type==='video'?'Videochamada recebida':'Chamada de voz recebida',incoming:true,video:false});
    if('Notification'in window&&Notification.permission==='granted'&&document.hidden){try{new Notification('Chamada no Link Direto',{body:`${name} está a ligar.`})}catch{}}
  }catch(err){if(!String(err).includes('404'))console.warn('incoming call poll',err)}
}
async function ldAcceptIncoming(){
  const row=ldCall.incoming;if(!row||!state.user)return;const name=ldCallName(row.caller_id);ldCallShow({name,status:'A ligar...',incoming:false,video:row.call_type==='video'});
  try{const stream=await ldCallMedia(row.call_type);ldCallAttachStream(stream);ldCall.current={id:row.id,proposalId:row.proposal_id,otherId:row.caller_id,kind:row.call_type,role:'callee',stream,pc:null,remoteIce:new Set(),connectedAt:null};ldCall.incoming=null;const pc=await ldCallCreatePeer(row.id,stream);ldCall.current.pc=pc;await pc.setRemoteDescription(new RTCSessionDescription(row.offer));const answer=await pc.createAnswer();await pc.setLocalDescription(answer);await api('call_sessions',`?id=eq.${row.id}`,{method:'PATCH',body:JSON.stringify({answer:pc.localDescription,status:'accepted',answered_at:new Date().toISOString()})});ldCallStartSessionPoll();ldCallStartIcePoll();ldCallSetStatus('A estabelecer ligação...')}
  catch(err){console.error('accept call',err);toast(err?.name==='NotAllowedError'?'Autoriza o microfone/câmara para atender.':'Não foi possível atender.',true);await ldEndCall(true)}
}
async function ldRejectIncoming(){const row=ldCall.incoming;if(!row)return;ldCall.incoming=null;ldCallHide();try{await api('call_sessions',`?id=eq.${row.id}`,{method:'PATCH',body:JSON.stringify({status:'rejected',ended_at:new Date().toISOString()})})}catch{}}
...
async function ldPollCurrentSession(){const c=ldCall.current;if(!c||!state.user)return;try{const rows=await api('call_sessions',`?id=eq.${c.id}&limit=1`),row=rows?.[0];if(!row)return;if(c.role==='caller'&&row.status==='accepted'&&row.answer&&!c.pc.remoteDescription){await c.pc.setRemoteDescription(new RTCSessionDescription(row.answer));ldCallSetStatus('A estabelecer ligação...')}if(row.status==='rejected'){toast('Chamada rejeitada.');ldCallCleanupLocal()}if(['ended','missed'].includes(row.status))ldCallCleanupLocal()}catch(err){console.warn('call session poll',err)}}
...
async function ldEndCall(notify=true){const c=ldCall.current;if(!c){if(ldCall.incoming)return ldRejectIncoming();ldCallHide();return}if(notify){try{await api('call_sessions',`?id=eq.${c.id}`,{method:'PATCH',body:JSON.stringify({status:'ended',ended_at:new Date().toISOString()})})}catch{}}ldCallCleanupLocal()}
function ldCallCleanupLocal(){ldCallStopTimers();const c=ldCall.current;if(c?.pc)try{c.pc.close()}catch{};if(c?.stream)c.stream.getTracks().forEach(t=>t.stop());ldCall.current=null;ldCall.incoming=null;const rv=$('ldRemoteVideo'),lv=$('ldLocalVideo');if(rv)rv.srcObject=null;if(lv)lv.srcObject=null;ldCallHide()}
...
function ldInitCalls(){if(ldCall.started)return;ldCall.started=true;ldCallEnsureOverlay();ldCall.pollTimer=setInterval(ldPollIncoming,1800);window.addEventListener('beforeunload',()=>{if(ldCall.current)ldEndCall(true)});document.addEventListener('visibilitychange',()=>{if(!document.hidden)ldPollIncoming()});}
